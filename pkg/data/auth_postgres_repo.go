...
func (repo *PostgresRepository) Create(ctx context.Context, user *User) error {
	...
	query := "insert into users (id, email, username, password, tokenhash, createdat, updatedat, isverified) values ($1, $2, $3, $4, $5, $6, $7, $8)"
	_, err := repo.db.ExecContext(ctx, query, user.ID, user.Email, user.Username, user.Password, user.TokenHash, user.CreatedAt, user.UpdatedAt, user.IsVerified)
	return err
}
