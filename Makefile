...
-include s3.env
S3_ENVPARMS:=  S3_ACCESS_KEY=${S3_ACCESS_KEY} S3_SECRET_KEY=${S3_SECRET_KEY} S3_HOST=${S3_HOST} S3_REGION=${S3_REGION}
...
.run:
	$(info #Running...)
	$(BUILD_ENVPARMS) $(AUTH_ENVPARMS) $(MAILGUN_ENVPARMS) $(S3_ENVPARMS) go run -ldflags "$(LDFLAGS)" ./cmd/${APP}
